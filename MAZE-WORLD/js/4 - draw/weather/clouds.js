...
let clouds = /** @type {HTMLElement[]} */ ([]);
let cloudShapes = /** @type {HTMLElement[]} */ ([]);
...
const setCloudShape = () => {
  ...
  cloudShapes.forEach((c) => {
    c.className = `cloud-shape ${POLYGONS_CLASS_MAP[currentPoly]}`;
    c.style.setProperty("--clouds-default-size", `${size}px`);
  });
};

const offsetClouds = () => {
  const { xSide } = getPolyInfo();
  const { jOffset } = POLY_INFO;
  const offsetSize = xSide * jOffset * -1.5;
...
const createClouds = () => {
  ...
  for (let i = 0; i < CLOUDS_CONFIG.num; i++) {
    ...
    const duration = Math.round(scale * 30);
